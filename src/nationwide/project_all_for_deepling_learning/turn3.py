...
# --- 4. Feature Engineering (Simplified - V3) ---
print("\n--- Step 4: Feature Engineering (V3 - Simplified) ---")
start_feat = time.time()
features_dict = {}
epsilon = 1e-6
RAIN_THR = 0.1 # Threshold for counting rain products

# Helper function for safe division (still needed for some calculations)
...
del day_index_original, day_of_year, month, season, seasons_onehot, season_expanded # Clean up
...
# 4.3.4 Moving Window Features (Keep as in v2, based on product mean - need product mean locally)
print("  Calculating moving window features (on local product mean)...")
# Recalculate product mean locally for window features
...
for window in [3, 7, 15]:
    ...
    # Need original X for window calculation
    for i in range(n_valid_days):
        ...
        # Calculate mean over products for the window data
        window_data_mean_prod = np.nanmean(window_data_X, axis=3, keepdims=True)
        # Calculate stats over time dimension of the product means
...
del product_mean_local # Delete local product mean
...
# 4.4.2 5x5 Neighborhood (Keep from v2)
...
# Keep spatial_center_diff_5x5
features_dict['spatial_center_diff_5x5'] = (features_dict['raw_values'] - spatial_mean_5x5).astype(np.float32)
del spatial_mean_5x5 # Can delete mean after diff calculation if not needed otherwise

# 4.4.3 Spatial Gradients (Keep from v2, but need product_mean locally)
print("    Spatial gradients...")
# Recalculate product mean locally if needed for gradient_magnitude_mean
...
del product_mean_local_grad, grad_lat_gsmap, grad_lon_gsmap, grad_lat_persiann, grad_lon_persiann, grad_lat_mean, grad_lon_mean # Clean up
...
# --- Delete lag stats caches --- (Moved earlier conceptually, but execute here)
...
# del spatial_center_diff_5x5 # Keep this one for now
...
for name, feat in features_dict.items():
    ...
    if n_cols == 1:
        feature_names.append(base_name)
         ...
         base_name.startswith('spatial_') and '_5x5' in base_name: # Only 5x5 spatial now
...
# Define output directory and filenames explicitly for v3
OUTPUT_DIR = "F:/rainfalldata"
X_flat_filename = os.path.join(OUTPUT_DIR, "X_flat_features_v3.npy")
Y_flat_filename = os.path.join(OUTPUT_DIR, "Y_flat_target_v3.npy")
feature_names_filename = os.path.join(OUTPUT_DIR, "feature_names_v3.txt")
...
for t in range(n_valid_days):
    ...
    col_idx = 0
    # --- Make sure features_dict only contains the features to be saved ---
    valid_feature_keys = list(features_dict.keys()) # Get keys before iteration
    for name in valid_feature_keys:
        if name not in features_dict: continue # Should not happen, but safe check
...
del X_flat_mmap
del Y_flat_mmap
del features_dict # Delete features dict after use
