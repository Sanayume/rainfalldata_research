...
# Use v4 data files
X_FLAT_PATH = os.path.join(PROJECT_DIR, "X_flat_features_v4.npy")  # v4
Y_FLAT_PATH = os.path.join(PROJECT_DIR, "Y_flat_target_v4.npy")  # v4
FEATURE_NAMES_PATH = os.path.join(PROJECT_DIR, "feature_names_v4.txt")  # v4
# Save v4 model and plot
MODEL_SAVE_PATH = os.path.join(PROJECT_DIR, "xgboost_v4_model.joblib")  # v4
IMPORTANCE_PLOT_PATH = os.path.join(PROJECT_DIR, "xgboost_v4_feature_importance.png")  # v4
...
# --- 1. 加载数据 ---
print("Loading flattened data (v4)...")
try:
...
# --- 4. 定义并训练 XGBoost 模型 ---
print("Defining and training XGBoost model (v4)...")
...
try:
    ...
    plt.ylabel("Feature")
    plt.title(f"Top {n_plot} Feature Importances (XGBoost v4)")
...
# --- 6. 评估模型 ---
print("\n--- Evaluating Model on Test Set (v4) ---")
...
for threshold in thresholds_to_evaluate:
    ...
    metrics = calculate_metrics(y_test, y_pred_threshold, title=f"XGBoost v4 (Threshold {threshold:.2f})")
    metrics_by_threshold[threshold] = metrics

print("\n--- XGBoost v4 Performance across different thresholds (Test Set) ---")
metrics_to_show = ['accuracy', 'pod', 'far', 'csi', 'fp', 'fn']
threshold_metrics_data = {}
for threshold, metrics in metrics_by_threshold.items():
    threshold_metrics_data[f'XGB_v4_Thr_{threshold:.2f}'] = {metric: metrics.get(metric, float('nan')) for metric in metrics_to_show}
