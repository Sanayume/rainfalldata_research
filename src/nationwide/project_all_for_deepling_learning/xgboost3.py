...
FEATURE_NAMES_PATH = os.path.join(PROJECT_DIR, "feature_names_v3.txt") # Path to v3 feature names file
# Save v3 model and plot
MODEL_SAVE_PATH = os.path.join(PROJECT_DIR, "xgboost_v3_model.joblib")
IMPORTANCE_PLOT_PATH = os.path.join(PROJECT_DIR, "xgboost_v3_feature_importance.png")

RAIN_THRESHOLD = 0.1
TEST_SIZE_RATIO = 0.2 # Keep the same split ratio for comparability
RANDOM_STATE = 42
EARLY_STOPPING_ROUNDS = 30
N_TOP_FEATURES_TO_PLOT = 50 # Plot top 50 or fewer if total features < 50
...
try:
    # Load full arrays into memory if possible, otherwise use mmap
...
if not isinstance(Y_flat_raw, np.memmap): # Don't delete if it's the mmap object itself
    del Y_flat_raw
...
# --- Attempting to use full training set ---
...
# Close memory map or delete array if loaded directly
if hasattr(X_flat, '_mmap'):
    X_flat._mmap.close()
    del X_flat # Delete mmap object proxy
elif 'X_flat' in locals() and not isinstance(X_flat, np.memmap):
     del X_flat # Delete array loaded directly
del y_flat_binary # Delete intermediate binary target

print(f"Training set shape: X={X_train.shape}, y={y_train.shape}")
print(f"Test set shape: X={X_test.shape}, y={y_test.shape}")

# --- 4. 定义并训练 XGBoost 模型 ---
print("Defining and training XGBoost model (v3)...")
...
try:
    ...
    plt.ylabel("Feature")
    plt.title(f"Top {n_plot} Feature Importances (XGBoost v3)")
...
# --- 6. 评估模型 ---
print("\n--- Evaluating Model on Test Set (v3) ---")
...
for threshold in thresholds_to_evaluate:
    ...
    metrics = calculate_metrics(y_test, y_pred_threshold, title=f"XGBoost v3 (Threshold {threshold:.2f})")
    metrics_by_threshold[threshold] = metrics

print("\n--- XGBoost v3 Performance across different thresholds (Test Set) ---")
metrics_to_show = ['accuracy', 'pod', 'far', 'csi', 'fp', 'fn']
threshold_metrics_data = {}
for threshold, metrics in metrics_by_threshold.items():
    threshold_metrics_data[f'XGB_v3_Thr_{threshold:.2f}'] = {metric: metrics.get(metric, float('nan')) for metric in metrics_to_show}
