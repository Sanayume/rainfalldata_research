import numpy as np
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
import os
import time

# --- 配置 ---
PROJECT_DIR = "F:/rainfalldata"
X_FLAT_PATH = os.path.join(PROJECT_DIR, "X_flat_features_v2.npy")
FEATURE_NAMES_V2_PATH = os.path.join(PROJECT_DIR, "feature_names_v2.txt")
FEATURE_NAMES_V1_PATH = os.path.join(PROJECT_DIR, "feature_names.txt") # Path to v1 feature names
CORRELATION_PLOT_PATH = os.path.join(PROJECT_DIR, "feature_correlation_v2_analysis.png")
CORRELATION_MATRIX_PATH = os.path.join(PROJECT_DIR, "feature_correlation_v2_matrix.csv") # Save matrix

N_SAMPLES_FOR_ANALYSIS = 1000000 # Analyze 1 million samples (adjust based on memory)
RANDOM_STATE = 42

# --- 1. 加载特征名称 ---
print("Loading feature names...")
try:
    with open(FEATURE_NAMES_V2_PATH, 'r') as f:
        feature_names_v2 = [line.strip() for line in f if line.strip()]
    print(f"Loaded {len(feature_names_v2)} v2 feature names.")

    with open(FEATURE_NAMES_V1_PATH, 'r') as f:
        feature_names_v1 = [line.strip() for line in f if line.strip()]
    print(f"Loaded {len(feature_names_v1)} v1 feature names.")
except Exception as e:
    print(f"Error loading feature names: {e}")
    exit()

# --- 2. 定义关键特征组 ---
# Based on user provided v2 results and general knowledge of v1 importance
# Top ~20 features from v2 results (adjust list as needed based on actual plot)
top_v2_features = [
    'spatial_mean_5x5_GSMAP', 'spatial_mean_5x5_PERSIANN', 'spatial_max_5x5_GSMAP',
    'spatial_max_5x5_PERSIANN', 'gradient_magnitude_GSMAP', 'gradient_magnitude_PERSIANN',
    'spatial_mean_5x5_IMERG', 'spatial_max_5x5_IMERG', 'lag_1_values_GSMAP',
    'lag_1_values_PERSIANN', 'spatial_mean_5x5_CMORPH', 'spatial_mean_5x5_CHIRPS',
    'spatial_max_5x5_CMORPH', 'spatial_max_5x5_CHIRPS', 'lag_2_values_GSMAP',
    'lag_2_values_PERSIANN', 'window_15_range', 'window_7_range',
    'gradient_magnitude_mean', 'lag_1_coef_of_variation'
]

# Features important in v1 but dropped significantly in v2 importance
# Check against feature_names_v1 and feature_names_v2 to ensure they exist in v2
dropped_v1_features_candidates = [
    'sin_day', 'cos_day', 'spatial_mean_3x3_GSMAP', 'spatial_mean_3x3_PERSIANN',
    'spatial_mean_3x3_CMORPH', 'spatial_mean_3x3_CHIRPS', 'lag_1_values_CMORPH',
    'lag_1_values_CHIRPS', 'lag_1_mean', 'lag_2_mean', 'lag_3_mean',
    'spatial_center_diff_3x3_GSMAP', 'spatial_center_diff_3x3_PERSIANN',
    'product_mean', 'product_std'
]
# Filter candidates to ensure they exist in the v2 feature list
dropped_v1_features = [f for f in dropped_v1_features_candidates if f in feature_names_v2]

print(f"\nTop V2 Features selected ({len(top_v2_features)}): {top_v2_features}")
print(f"Dropped V1 Features selected ({len(dropped_v1_features)}): {dropped_v1_features}")

# Combine lists and get unique features for analysis
features_to_analyze = sorted(list(set(top_v2_features + dropped_v1_features)))
print(f"\nTotal unique features for correlation analysis ({len(features_to_analyze)}): {features_to_analyze}")

# Get indices of these features in the v2 list
try:
    feature_indices = [feature_names_v2.index(f) for f in features_to_analyze]
except ValueError as e:
    print(f"Error: One of the selected features not found in v2 feature list: {e}")
    exit()

# --- 3. 加载数据子集 ---
print(f"\nLoading data subset ({N_SAMPLES_FOR_ANALYSIS} samples)...")
start_load = time.time()
try:
    X_flat_mmap = np.load(X_FLAT_PATH, mmap_mode='r')
    n_total_samples = X_flat_mmap.shape[0]
    n_features_total = X_flat_mmap.shape[1]

    if n_total_samples < N_SAMPLES_FOR_ANALYSIS:
        print(f"Warning: Requested sample size ({N_SAMPLES_FOR_ANALYSIS}) > total samples ({n_total_samples}). Using all samples.")
        N_SAMPLES_FOR_ANALYSIS = n_total_samples
        sample_indices = np.arange(n_total_samples)
    else:
        np.random.seed(RANDOM_STATE)
        sample_indices = np.random.choice(n_total_samples, N_SAMPLES_FOR_ANALYSIS, replace=False)
        sample_indices.sort() # Sorting might improve read performance slightly

    # Load selected rows first, then select columns
    print("Reading selected rows...")
    X_subset_rows = X_flat_mmap[sample_indices]
    print("Selecting feature columns...")
    X_subset = X_subset_rows[:, feature_indices]

    # Close mmap
    if hasattr(X_flat_mmap, '_mmap'):
        X_flat_mmap._mmap.close()
    del X_flat_mmap, X_subset_rows # Free memory

    print(f"Loaded data subset shape: {X_subset.shape}")
    end_load = time.time()
    print(f"Data loading finished in {end_load - start_load:.2f} seconds.")

except Exception as e:
    print(f"Error loading data subset: {e}")
    exit()

# --- 4. 计算和分析相关性 ---
print("\nCalculating correlation matrix...")
start_corr = time.time()
df_subset = pd.DataFrame(X_subset, columns=features_to_analyze)
del X_subset # Free memory

# Calculate correlation matrix
correlation_matrix = df_subset.corr()
end_corr = time.time()
print(f"Correlation calculation finished in {end_corr - start_corr:.2f} seconds.")

# Save the full correlation matrix
try:
    print(f"Saving correlation matrix to {CORRELATION_MATRIX_PATH}...")
    correlation_matrix.to_csv(CORRELATION_MATRIX_PATH)
except Exception as e:
    print(f"Error saving correlation matrix: {e}")

# Analyze correlations between Top V2 and Dropped V1 features
print("\n--- Correlations between Top V2 and Dropped V1 Features ---")
# Select the relevant part of the correlation matrix
corr_submatrix = correlation_matrix.loc[dropped_v1_features, top_v2_features]

# Find and print high correlations (e.g., absolute value > 0.7)
high_corr_pairs = corr_submatrix[corr_submatrix.abs() > 0.7].stack().reset_index()
high_corr_pairs.columns = ['Dropped_V1_Feature', 'Top_V2_Feature', 'Correlation']
# Remove self-correlations if any feature is in both lists (shouldn't happen with current selection)
high_corr_pairs = high_corr_pairs[high_corr_pairs['Dropped_V1_Feature'] != high_corr_pairs['Top_V2_Feature']]

if not high_corr_pairs.empty:
    print("High Correlation Pairs (|Correlation| > 0.7):")
    print(high_corr_pairs.to_string())
else:
    print("No high correlations (|Correlation| > 0.7) found between the selected groups.")

# --- 5. 可视化 ---
print("\nGenerating correlation heatmap...")
try:
    plt.figure(figsize=(max(15, len(features_to_analyze)*0.4), max(12, len(features_to_analyze)*0.4))) # Adjust size dynamically
    sns.heatmap(correlation_matrix, cmap='coolwarm', annot=False, fmt=".2f", linewidths=.5) # Annot=False for large matrices
    plt.title(f'Feature Correlation Matrix (V2 Analysis - {N_SAMPLES_FOR_ANALYSIS} samples)')
    plt.xticks(rotation=90)
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(CORRELATION_PLOT_PATH)
    print(f"Correlation heatmap saved to: {CORRELATION_PLOT_PATH}")
    plt.close()
except Exception as e:
    print(f"Error generating heatmap: {e}")

print("\nAnalysis finished.")

