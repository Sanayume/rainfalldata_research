...
# 定义数据路径和常量
chirpsfolder = os.path.join(basepath, "raw","nationwide", "CHIRPS")
...
for count, (year, year_str) in enumerate(year_info):
    ...
    # 保存数据
    output_file = os.path.join(savepath, f'chirps_{year_str}.mat')
...
if input("\n是否需要合并所有年份数据？(y/n): ").lower() == 'y':
    ...
    for _, year_str in year_info:
        data = loadmat(os.path.join(savepath, f'chirps_{year_str}.mat'))['data']
        all_data.append(data)
    
    all_data = np.concatenate(all_data, axis=2)
    savemat(os.path.join(savepath, 'chirps_2016_2020.mat'), {'data': all_data})
    print("✓ 合并完成！")
