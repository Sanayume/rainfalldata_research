...
for start_idx, end_idx, year in year_info:
    ...
    # 保存结果
    print(f"保存{year}年结果...")
    savemat(os.path.join(savepath, f'IMERG_{year}.mat'), {'data': data_025deg})
...
if input().lower() == 'y':
    ...
    for _, _, year in year_info:
        data = loadmat(os.path.join(savepath, f'IMERG_{year}.mat'))['data']
        all_data.append(data)
    
    all_data = np.concatenate(all_data, axis=2)
    savemat(os.path.join(savepath, 'IMERG_2016_2020.mat'), {'data': all_data})
    print("合并完成！")
