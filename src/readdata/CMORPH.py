...
# 定义数据路径和常量
savepath = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "intermediate", "nationwide", "CMORPHdata")
basepath = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data")
if not os.path.exists(savepath):
    os.makedirs(savepath)

cmorphfolder = os.path.join(basepath, "raw", "nationwide", "CMORPH")
...
for start_idx, end_idx, year in year_info:
    ...
    # 保存结果
    savemat(os.path.join(savepath, f'CMORPH_{year}.mat'), {'data': china_data})
    print(f"✓ {year}年数据已保存")
...
if input("\n是否需要合并所有年份数据？(y/n): ").lower() == 'y':
    ...
    for _, _, year in year_info:
        data = loadmat(os.path.join(savepath, f'CMORPH_{year}.mat'))['data']
        all_data.append(data)
    
    all_data = np.concatenate(all_data, axis=2)
    savemat(os.path.join(savepath, 'CMORPH_2016_2020.mat'), {'data': all_data})
    print("✓ 合并完成！")
