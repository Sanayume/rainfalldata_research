...
# -*- coding: utf-8 -*-
"""
TN专家模型Optuna优化结果读取脚本
==================================

读取并分析TN专家模型的Optuna超参数优化结果
...
def read_tn_expert_optuna_results():
    """读取TN专家模型的Optuna优化结果"""
    
    # 设置路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    optuna_db_path = os.path.join(current_dir, "Ensemble_v2", "optuna_db", "tn_expert_optimization.db")
    
    print("=" * 80)
    print("TN专家模型 - Optuna优化结果分析")
    print("=" * 80)
    ...
    try:
        ...
        # 保存试验结果CSV
        csv_path = os.path.join(output_dir, "tn_expert_optuna_trials.csv")
        df_trials.to_csv(csv_path, index=False)
        print(f"\n试验结果已保存到: {csv_path}")
        
        # 保存最佳参数
        best_params_path = os.path.join(output_dir, "tn_expert_best_params.txt")
        ...
        if len(completed_trials) > 1:
            ...
            # 子图1: 优化历史
            plt.subplot(2, 2, 1)
            plt.plot(completed_trials['number'], completed_trials['value'], 'm-', alpha=0.7)
            plt.scatter(completed_trials['number'], completed_trials['value'], c=completed_trials['value'], cmap='magma', s=30)
            plt.xlabel('Trial Number')
            plt.ylabel('AUC Score')
            plt.title('TN Expert Optimization History')
            plt.grid(True, alpha=0.3)
            
            # 子图2: AUC分布直方图
            plt.subplot(2, 2, 2)
            plt.hist(completed_trials['value'], bins=30, alpha=0.7, color='plum', edgecolor='black')
            ...
            if completed_trials['duration'].notna().any():
                ...
                plt.scatter(valid_durations['number'], valid_durations['duration'], alpha=0.6, color='purple')
                plt.xlabel('Trial Number')
            ...
            plt.plot(completed_trials['number'], cumulative_best, 'm-', linewidth=2)
            plt.xlabel('Trial Number')
            ...
            # 保存图片
            plot_path = os.path.join(output_dir, "tn_expert_optimization_analysis.png")
            plt.savefig(plot_path, dpi=300, bbox_inches='tight')
            print(f"优化分析图已保存到: {plot_path}")
            plt.show()
        
        print("\n" + "=" * 80)
        print("TN专家模型Optuna结果分析完成!")
        print("=" * 80)
