...
# --- 配置 ---
PROJECT_DIR = "F:/rainfalldata/YangTsu/" # Changed to Yangtze directory
# Use Yangtze v3 data files
X_FLAT_PATH = os.path.join(PROJECT_DIR, "X_Yangtsu_flat_features_v3.npy") # Changed filename
Y_FLAT_PATH = os.path.join(PROJECT_DIR, "Y_Yangtsu_flat_target_v3.npy") # Changed filename
FEATURE_NAMES_PATH = os.path.join(PROJECT_DIR, "feature_names_yangtsu_v3.txt") # Changed filename
# Save Yangtze v3 model and plot
MODEL_SAVE_PATH = os.path.join(PROJECT_DIR, "xgboost_yangtsu_v3_model.joblib") # Changed filename
IMPORTANCE_PLOT_PATH = os.path.join(PROJECT_DIR, "xgboost_yangtsu_v3_feature_importance.png") # Changed filename
PERFORMANCE_CSV_PATH = os.path.join(PROJECT_DIR, "threshold_performance_yangtsu_v3.csv") # Changed filename
...
# --- 1. 加载数据 ---
print("Loading flattened Yangtze data (v3)...")
try:
    ...
    print("Run turn3.py for Yangtze first.")
    exit()
...
# --- 4. 定义并训练 XGBoost 模型 ---
print("Defining and training XGBoost model (Yangtze v3)...")
...
params = {
    ...
    'max_depth': 7,
    'subsample': 0.9,
    'colsample_bytree': 0.8, # Adjusted from 0.7 in v2 example, keep consistent or tune
    'gamma': 0.2,
    'random_state': RANDOM_STATE,
...
# --- 5. 特征重要性 ---
print("\n--- Feature Importances (Yangtze v3) ---")
try:
    ...
    plt.ylabel("Feature")
    plt.title(f"Top {n_plot} Feature Importances (XGBoost Yangtze v3)") # Updated title
...
# --- 6. 评估模型 ---
print("\n--- Evaluating Model on Test Set (Yangtze v3) ---")
...
for threshold in thresholds_to_evaluate:
    ...
    metrics = calculate_metrics(y_test, y_pred_threshold, title=f"XGBoost Yangtze v3 (Threshold {threshold:.2f})") # Updated title
    metrics_by_threshold[threshold] = metrics

print("\n--- XGBoost Yangtze v3 Performance across different thresholds (Test Set) ---") # Updated title
metrics_to_show = ['accuracy', 'pod', 'far', 'csi', 'fp', 'fn']
threshold_metrics_data = {}
for threshold, metrics in metrics_by_threshold.items():
    threshold_metrics_data[f'XGB_Yangtsu_v3_Thr_{threshold:.2f}'] = {metric: metrics.get(metric, float('nan')) for metric in metrics_to_show} # Updated row names
