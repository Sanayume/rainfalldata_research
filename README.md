...
*   **V1 (`turn1.py`): 特征体系初建 (针对点位数据)**
    *   **基础特征**: 各降雨产品在目标点的原始降雨量 (`raw_values`)。
    *   **多产品协同 (当前时刻)**: 原始值的均值 (`product_mean`)、标准差 (`product_std`)、中位数 (`product_median`)、最大值 (`product_max`)、最小值 (`product_min`)、极差 (`product_range`)，以及当前时刻降雨产品数 (`rain_product_count`，基于 `RAIN_THR=0.1mm` 阈值)。
    *   **时序动态捕捉**:
    ...
    *   **交互特征**: `product_std * sin_day`, `low_intensity_std * coef_of_variation`, `rain_product_count * product_std`。
    *   **数据处理**: 输入为流域点位数据，通过 `ALL_DATA.get_basin_point_data(basin_mask_value=2)` 获取。

*   **V2 (`turn2.py`): 特征体系深化与扩展 (针对点位数据)**
    *   **基于 V1 进行扩展。**
    *   **记忆性 (滞后项) 深化**:
        *   为 t-1, t-2, t-3 天的滞后项增加了降雨产品数 (`lag_X_rain_count`)。
        *   增加了滞后项之间的均值差分，如 `lag_1_2_mean_diff` (t-1均值 - t-2均值)。
    *   **累积效应 (滑动窗口) 深化**:
        *   在原有基于产品均值的滑动统计基础上，增加了针对特定产品（GSMAP 的均值，PERSIANN 的标准差）的 7 天滑动窗口统计 (`window_7_mean_GSMAP`, `window_7_std_PERSIANN`)。
    *   **空间关联 (点位数据的名义空间特征) 深化**:
        *   增加了名义上的 5x5 邻域统计量 (均值、标准差、最大值、中心点差异，仍为 NaN 占位符)。
        *   增加了名义上的空间梯度特征 (GSMAP梯度、PERSIANN梯度、平均梯度，仍为 NaN 占位符)。
    *   **弱信号增强 (强化)**:
        *   增加了滞后一天 (t-1) 的变异系数 (`lag_1_coef_of_variation`)。
        *   增加了基于**降雨产品数量**的特定强度分箱 (`intensity_bins_count`)，原有基于产品均值的分箱 (`intensity_bins_mean`) 予以保留。
    *   **交互特征 (强化)**:
        *   增加了 `product_std * spatial_center_diff` (仍为 NaN 占位符)。
        *   增加了 `product_std * diff_1_mean`。
    *   **数据加载方式微调**: `ALL_DATA.yangtsu()`，但本质上应该还是点位数据。

*   **V3 (`turn3.py`): 特征体系简化探索 (针对点位数据)**
    *   **显著简化特征集，旨在测试核心特征的有效性。**
    *   **多产品协同 (简化)**: 仅保留 `rain_product_count`。移除了均值、标准差、中位数、最值、极差等。
    *   **时序动态捕捉 (简化)**:
        *   **周期性**: 仅保留 `season_onehot`。移除了 `sin_day`, `cos_day`。
        *   **记忆性 (滞后项)**: 保留 `lag_X_values`, `lag_X_std`, `lag_X_rain_count`。移除了 `lag_X_mean` 和滞后项间的均值差分。
        *   **变化率**: 仅保留 `diff_1_values`。移除了 `diff_1_mean`, `diff_1_std`。
        *   **累积效应 (滑动窗口)**: 保留了 V2 中的所有滑动窗口特征（基于产品均值的，以及特定产品的）。
    *   **空间关联 (点位数据的名义空间特征) (简化)**:
        *   移除了名义上的 3x3 邻域特征。
        *   保留了名义上的 5x5 邻域特征和空间梯度特征 (仍为 NaN 占位符)。
    *   **弱信号增强 (大幅简化)**:
        *   仅保留基于**降雨产品数量**的特定强度分箱 (`intensity_bins_count`)。
        *   移除了 `threshold_proximity`, `coef_of_variation` (当前和滞后), `low_intensity_std`, 以及基于产品均值的强度分箱。
    *   **交互特征**: **全部移除**。

*   **V4 (`turn4.py`): 在简化基础上恢复部分周期特征 (针对点位数据)**
    *   **基本等同于 V3 的特征集。**
    *   **核心变化**: 重新引入了年内周期正余弦编码 (`sin_day`, `cos_day`)。其他特征与 V3 保持一致。

*   **V5 (`turn5.py`): 在 V3 基础上重新引入部分关键信号特征 (针对点位数据)**
    *   **特征集更接近 V3，但有所增补，并未继承 V4 的 `sin_day`, `cos_day`。**
    *   **多产品协同 (增强)**: 在 V3 的 `rain_product_count` 基础上，重新引入了 `coef_of_variation` 和 `product_range` (这意味着内部计算了均值、标准差、最值、最小值，但未直接作为独立特征输出)。
    *   **时序动态捕捉**: 与 V3/V4 类似。
        *   **周期性**: 仅 `season_onehot`。
        *   **记忆性 (滞后项)**: `lag_X_values`, `lag_X_std`, `lag_X_rain_count`。
        *   **变化率**: `diff_1_values`。
        *   **累积效应 (滑动窗口)**: 与 V2/V3/V4 一致。
    *   **空间关联**: 与 V3/V4 一致 (名义 5x5 及梯度，NaN 占位符)。
    *   **弱信号增强 / 模糊性特征 (增强)**:
        *   重新引入 `threshold_proximity`。
        *   新增 `fraction_products_low_range` (降雨量在 (0, 0.5mm] 区间的产品比例)。
        *   保留 `intensity_bins_count`。

*   **V5.1 (`turn5_1.py`): 重大转变 - 转向格点数据并大幅简化特征**
    *   **数据处理重大变化**: `ALL_DATA.yangtsu()` 返回的是**格点数据** (`(prod, time, lat, lon)`) 而非之前的点位数据。脚本中明确处理了经纬度维度，并在展平时应用了有效格点掩码 (`valid_mask`)。使用了 `memmap` 处理大规模格点数据。
    *   **特征集大幅简化，适配格点数据特性，并可能为后续深度学习模型做准备。**
    *   **基础特征**: `raw_values`。
    *   **多产品协同**: 仅 `rain_product_count`。
    *   **时序动态捕捉**:
        *   **周期性**: 仅 `season_onehot`。
        *   **记忆性 (滞后项)**: 仅 `lag_X_values`。移除了滞后标准差和滞后降雨产品数。
        *   **变化率**: `diff_1_values`。
        *   **累积效应 (滑动窗口)**: 保留了先前版本中基于多产品日均值的 3, 7, 15 天滑动窗口统计 (均值、标准差、最值、范围)。**注意：特定产品的滑动窗口特征被移除。**
    *   **空间关联 (针对格点数据真实计算)**:
        *   **5x5 邻域**: 针对每个产品，真实计算了中心格点周围 5x5 邻域的均值 (`spatial_mean_5x5`)、标准差 (`spatial_std_5x5`)、最大值 (`spatial_max_5x5`)。**注意：邻域中心点差异特征 (`spatial_center_diff_5x5`) 被移除。**
        *   移除了原先名义上的空间梯度特征。
    *   **弱信号增强 / 模糊性特征**: **全部移除**。
    *   **交互特征**: **全部移除**。
...
这一系列迭代清晰地展示了从构建复杂特征集 (V1, V2)，到尝试简化和聚焦核心特征 (V3, V4, V5)，最终转向处理格点数据并为可能更复杂的模型（如深度学习）准备更基础、但包含真实空间信息的特征集 (V5.1) 的演进路径。
...
#### 2.3.1 长江流域 XGBoost 模型性能迭代与评估实例

针对长江流域，基于不同版本的特征工程（详见 2.2 节中 V1-V5.1 的迭代描述），我们使用 XGBoost 作为主力分类模型进行了训练和评估。以下表格汇总了部分关键版本的模型在**测试集**上的性能表现，均采用二分类（有雨/无雨，阈值 > 0.1mm/d 判为有雨），并展示了不同预测概率阈值下的核心气象评估指标。

**1. 特征集 V1 (基于 `turn1.py`):**

*   **默认参数模型 (`xgboost1 - 副本.py`):**
    *   在预测概率阈值为 0.5 时：
...
*   **Optuna 优化参数模型 (`xgboost1.py`):**
    *   在预测概率阈值为 0.5 时：
        *   Accuracy: 0.9292, POD: 0.9237, **FAR: 0.0399**, CSI: 0.8896
        *   FP: 25069, FN: 49830
    *   模型在不同阈值下的表现：

        | 预测阈值 | Accuracy | POD    | FAR    | CSI    | FP    | FN     |
        |----------|----------|--------|--------|--------|-------|--------|
        | 0.40     | 0.9307   | 0.9406 | 0.0532 | 0.8933 | 34542 | 38794  |
        | 0.45     | 0.9304   | 0.9322 | 0.0460 | 0.8922 | 29322 | 44259  |
        | **0.50** | **0.9292** | **0.9237** | **0.0399** | **0.8896** | **25069** | **49830**  |
        | 0.55     | 0.9270   | 0.9145 | 0.0346 | 0.8855 | 21381 | 55843  |
        | 0.60     | 0.9240   | 0.9046 | 0.0296 | 0.8803 | 18038 | 62298  |
        | 0.65     | 0.9197   | 0.8933 | 0.0254 | 0.8730 | 15186 | 69705  |
        | 0.70     | 0.9145   | 0.8809 | 0.0214 | 0.8642 | 12603 | 77809  |

    *   与原始卫星产品相比 (在测试集，概率阈值 0.5，降雨阈值 0.1mm/d)：

        | 模型/产品                            | Accuracy | POD    | FAR    | CSI    | FP      | FN      |
        |--------------------------------------|----------|--------|--------|--------|---------|---------|
        | XGBoost_Yangtsu_Optimized (Thr 0.5)  | 0.9292   | 0.9237 | 0.0399 | 0.8896 | 25069   | 49830   |
        | Baseline_CMORPH_Yangtsu              | 0.6334   | 0.5168 | 0.1763 | 0.4654 | 361158  | 1577665 |
        | Baseline_CHIRPS_Yangtsu              | 0.5906   | 0.4413 | 0.1913 | 0.3996 | 340810  | 1824393 |
        | Baseline_SM2RAIN_Yangtsu             | 0.6930   | 0.9033 | 0.3072 | 0.6450 | 1307610 | 315866  |
        | Baseline_IMERG_Yangtsu               | 0.7145   | 0.7073 | 0.1935 | 0.6047 | 554123  | 955706  |
    ...
    *   **关键观察**: Optuna 超参数优化显著提升了各项指标，特别是将 FAR 从约 0.082 降低到了约 **0.040**，CSI 从 0.823 提升到了 **0.890** (在阈值0.5时)。

**2. 基于简化/调整后特征集的 XGBoost 模型性能:**

以下为基于后续 `turn2.py` 至 `turn5.py` 生成的特征集训练的 XGBoost 模型（使用默认或预设参数，未进行针对性的 Optuna 优化）在不同概率阈值下的性能表现。这些结果主要用于观察特征集变化对模型性能的趋势性影响。

*   **特征集 V2 (`turn2.py`, 模型 `xgboost2.py`):**
...
*   **特征集 V3 (`turn3.py`, 模型 `xgboost3.py`):**
...
*   **特征集 V4 (`turn4.py`, 模型 `xgboost4.py`):**
...
*   **特征集 V5 (`turn5.py`, 模型 `xgboost5.py`):**
...
*   **特征集 V5.1 (`turn5_1.py`, 模型 `xgboost5_1.py`):** (*此版本特征工程转向格点数据，结果代表了不同数据处理方式下的性能*)

    | 预测阈值 | Accuracy | POD    | FAR    | CSI    | FP    | FN     |
    |----------|----------|--------|--------|--------|-------|--------|
    | 0.45     |  -       |   -    |   -    |   -    |   -   |   -    |
    | **0.50** | **0.9355** | **0.9301** | **0.0366** | **0.9004** | **23156** | **46476**  |
    | 0.55     |  -       |   -    |   -    |   -    |   -   |   -    |
    *(注: V5.1 的完整多阈值结果在原始输出中未完全对应，此处仅列出阈值0.5的关键指标作为参考。根据 `xgboost5_1.py` 脚本本身输出，其在0.5阈值下 POD 0.9301, FAR 0.0366, CSI 0.9004)*

*   **特征集 V1 + Optuna 超参数优化 + 5 折交叉验证 (`xgboost1.py` - 修改版):**
    *   **优化过程:** 使用 Optuna 在 V1 特征集的训练数据子集上进行了 50 次试验（超时时间 360000 秒，约 100 小时，实际用时约 8.7 小时），优化目标为 `auc`。
    *   **找到的最佳超参数:**
        ```python
        {
            'n_estimators': 1600,
    ...
    *   **K-Fold 交叉验证:** 使用上述优化参数，在除去最终保持测试集后的训练数据池 (`Training/CV Pool`) 上进行了 5 折 `StratifiedKFold` 交叉验证。
        *   每折模型都进行了训练并保存。
        *   生成了覆盖整个 `Training/CV Pool` 的**折外预测概率** (`Train_L0_Probs_V1_Opt.npy`)，这是集成学习第二阶段的关键输入。
    ...
    *   **关键观察 (基于 Hold-out Test Set 结果):**
        *   经过 Optuna 超参数优化和规范的训练流程后，V1 特征集训练的 XGBoost 模型在独立的保持测试集上表现出色。
    ...
    *   **产出文件 (用于集成学习):**
        *   折外预测概率 (Training/CV Pool): `f:\rainfalldata\results\yangtze\features\kfold_optimization_v1\Train_L0_Probs_V1_Opt.npy`
        *   最终模型在保持测试集上的预测概率: `f:\rainfalldata\results\yangtze\features\kfold_optimization_v1\Test_L0_Probs_V1_Opt_from_final_model.npy`
        *   最终优化模型: `f:\rainfalldata\results\yangtze\features\kfold_optimization_v1\xgboost_v1_opt_final_model_on_train_cv_pool.joblib`
        *   各折模型位于 `kfold_optimization_v1` 目录下。

**总结与洞察 (更新):**

*   **V1 特征集 + Optuna 优化 + 规范 K-Fold 流程** 构建的 XGBoost 模型在独立的保持测试集上展现了当前项目中最优的综合性能，尤其是在误报率 (FAR) 控制方面取得了显著成效 (在阈值0.5时FAR约为0.036，阈值0.7时FAR可低至0.021)。
*   此模型的折外预测 (`Train_L0_Probs_V1_Opt.npy`) 是进行后续集成学习以进一步优化 FP/FN 的坚实基础。
*   GSMAP 相关特征和多产品一致性特征在模型决策中起到了关键作用。
**总结与洞察:**

*   **Optuna 优化的重要性**: 对比特征集 V1 的默认参数和优化参数模型，超参数优化能显著提升模型性能，特别是在降低 FAR 和提高 CSI 方面。
*   **特征集迭代影响**:
    *   V2 相对于 V1（优化后）在类似参数下，FAR 略有上升，CSI 略有下降，表明 V2 增加的特征可能引入了一些噪声或冗余。
    *   V3 (简化特征集) 相对于 V2，FAR 进一步上升，CSI 下降，说明过度简化特征可能损失了部分有效信息。
    *   V4 (V3 + sin/cos) 相对于 V3，FAR 略有降低，CSI 略有提升，显示了周期性特征的价值。
    *   V5 (V3 + 部分关键信号) 相对于 V4，在 FAR 和 CSI 上表现略逊于 V4，说明增加的特征可能未带来显著正向收益，或者需要更精细的调参。
    *   **V5.1 (格点数据 + 大幅简化特征)** 在阈值 0.5 时表现出了最佳的 FAR (0.0366) 和 CSI (0.9004)，这突显了**转向包含真实空间信息的格点数据处理方式**以及针对此类数据设计的特征集的重要性。这可能是未来模型优化的一个关键方向。
*   **FAR 的挑战**: 即使是优化后的模型，FAR 仍然是需要重点关注和改进的指标。后续的集成学习策略将重点针对此问题。
*   **模型对比基准**: XGBoost 模型（尤其是经过优化和使用格点特征V5.1）显著优于所有单一的卫星遥感产品基线。

这些结果为后续的模型选择、特征优化方向（如进一步挖掘格点数据的空间特性）以及集成学习策略的设计提供了宝贵的经验和数据支持。
