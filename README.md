...
精准的降雨信息是洪水预警、农业生产规划、水资源可持续管理等多个国计民生领域不可或缺的关键输入。然而，现有的单一降雨数据源，无论是地面观测还是卫星遥感，往往在时空覆盖完整性、数据精度以及特定降雨事件的捕捉能力方面存在固有的局限性。例如，地面站点分布不均难以刻画精细的空间降雨分布，而卫星遥感产品则可能受到反演算法、传感器特性以及复杂下垫面环境的影响，导致估算偏差。这些问题直接制约了依赖降雨数据的下游应用的效能和可靠性。

本项目针对上述挑战，旨在通过系统性地融合多种主流卫星遥感降雨产品（如CMORPH, CHIRPS, GSMAP, IMERG, PERSIANN, SM2RAIN）与高质量的地面观测融合数据（如CHM），并引入数字高程模型（DEM）等关键地理空间数据以及其他相关的气象要素，充分利用它们在信息源、物理机制和时空特性上的互补性。在此基础上，项目将深度结合前沿的机器学习与深度学习技术，探索并构建一套高性能、高分辨率的降雨融合与预测新范式。

本项目的核心价值在于，通过多源数据融合与先进智能算法的结合，不仅致力于显著提升降雨估测与预测的精度，特别是在降雨事件的命中率、误报率及临界成功指数等关键业务指标上寻求突破，也期望深入揭示数据融合、特征工程和机器学习在解决复杂气象水文问题中的巨大潜力。研究成果有望为相关领域的科研工作提供具有参考价值的方法论体系和坚实的技术积累，并为发展更可靠的业务化降雨产品奠定基础。

### 1.2 研究目标与核心任务

为实现上述愿景，本项目设定了以下主要研究目标和核心技术任务：

1.  **多源异构数据整合与预处理流程构建：**
    *   开发一套稳健、高效的数据处理流程，能够自动化地获取、清洗、时空对齐并有效整合来自多种卫星遥感降雨产品、地面观测融合数据以及DEM、其他气象要素等多源异构数据。
    *   解决数据在格式、分辨率、投影、时间基准等方面的不一致问题，确保数据质量和后续分析的可靠性。

2.  **降雨预测任务分解与分层建模策略设计：**
    *   将复杂的降雨预测问题分解为更易于处理的子任务，例如，首先进行“有无降雨”的分类判断，再对判断为“有雨”的区域/时段进行降雨量的定量回归预测。
    *   针对不同子任务的特性，选择或设计最适合的机器学习/深度学习模型架构。
    *   研究并实现有效的Stacking或其他集成学习机制，将各子任务模型的输出进行智能融合，以获得最终的高性能降雨产品。

3.  **大规模、多维度特征工程体系的构建与优化：**
    *   基于对降雨物理过程的理解和数据驱动的探索，设计并实现一个大规模、多维度的特征库。
    *   特征将涵盖：各数据源的基础信息、多产品协同统计量、丰富的时序动态特征（周期性、滞后性、变化率、累积效应）、基于DEM等辅助数据的空间关联特征、以及针对弱信号（如毛毛雨）的增强特征和高阶交互特征。
    *   建立特征迭代优化机制，根据模型反馈持续 refining 特征集。

4.  **机器学习/深度学习模型探索、评估与优化：**
    *   系统性地调研、选择、实现并评估多种先进的机器学习模型（如梯度提升树XGBoost, LightGBM；贝叶斯方法等）和深度学习框架下的神经网络模型（如适用于时空序列预测的ConvLSTM等）。
    *   针对不同子任务（分类、回归）和不同模型，进行细致的超参数优化。
    *   重点关注模型在降雨命中率（POD）、误报率（FAR）和临界成功指数（CSI）等关键气象评估指标上的表现，力求在误报率控制上取得突破。

5.  **模型误差深度诊断与迭代改进：**
    *   建立全面的模型评估与诊断体系，深入分析模型预测误差的来源（如误报FP、漏报FN样本的特征分布、时空分布规律等）。
    *   将误差分析的结果反馈到特征工程和模型结构设计中，驱动整个研究框架的持续迭代优化。

6.  **区域精细化建模与可推广性探索：**
    *   首先选择重点区域（如长江流域）进行精细化的数据处理、特征工程和模型构建与验证。
    *   在重点区域取得经验和方法突破后，逐步将研究框架和模型体系推广到其他具有代表性的流域（如黄河流域、珠江流域），并最终扩展至更广阔的区域（如中国东部乃至全国），探索区域适应性预测策略和模型的泛化能力。
