...
3.  **机器学习与深度学习模型的精细化应用与创新集成：**
    ...
    *   **先进集成学习策略：** 系统性研究并应用如Stacking等高级集成学习框架，将不同子任务模型（分类器、回归器、误差修正模型等）的输出作为元特征，训练更高层次的元学习器，以实现对最终降雨预测结果的优化整合与性能提升。
        *   **第二次尝试：两阶段四元特征融合（Ensemble_v2）**：在 `src/yangtze/Ensemble_v2/` 目录下，我们探索了一种创新的两阶段集成学习策略。该策略旨在通过学习基础模型的误差模式来对预测进行修正。

            **核心思想**：训练四个“专家模型”，每个模型专门学习一种预测情景：
            1.  **TP专家**：学习基础模型正确预测为“有雨”的样本特征。
            2.  **TN专家**：学习基础模型正确预测为“无雨”的样本特征。
            3.  **FP专家**：学习基础模型错误预测为“有雨”（误报）的样本特征。
            4.  **FN专家**：学习基础模型错误预测为“无雨”（漏报）的样本特征。

            **方法论缺陷与经验教训**：
            在实践中，我们发现该策略的初始实现存在一个严重的方法论问题——**数据泄露**。专家模型的训练目标（例如，一个样本是否为TP）是通过对比基础模型的预测和**真实标签（y_true）**来生成的。这意味着专家模型在训练时“看到”了最终答案，导致其性能被严重高估，无法泛化到未知数据。

            > 这是一个关键的教训：在任何分层或元学习架构中，必须确保每一层模型的训练数据严格独立于下一层将要预测的目标信息。

            **探索的有价值部分**：
            尽管整体架构存在缺陷，但这次探索过程产出了许多有价值的子模块和经验，特别是针对FP/FN专家训练中遇到的**极端类别不平衡**问题：
            *   **高级数据平衡策略**：开发了多种数据平衡脚本（如 `train_balanced_fp_fn_experts.py`），实现了混合采样（Hybrid）、过采样（Oversampling）和欠采样（Undersampling）等多种策略，并探索了集成多个平衡子集的健壮训练方法。
            *   **贝叶斯网络建模**：探索了使用贝叶斯网络（`baysian_network_fp_fn.py`）作为专家模型，并设计了基于气象学原理的复杂网络结构，为处理不确定性和因果推断提供了新的思路。
            *   **FN专家深度优化**：针对最难处理的FN（漏报）问题，进行了专门的特征工程、评估指标设计和集成策略研究（如 `fn_expert_optimizer.py`, `fn_ensemble_strategy.py`）。

            **最终专家模型独立性能（修正后）**：
            在修正了评估流程，确保在独立交叉验证集上进行评估后，各专家模型的真实性能如下（基线模型XGBoost AUC为 **0.9887**）：

            | 专家类型 | 5折平均AUC | OOF AUC | 与基线对比 | 状态 |
            |:---:|:---:|:---:|:---:|:---:|
            | **TP专家** | 0.9947 | 0.9947 | +0.0060 | ✅ 超越基线 |
            | **TN专家** | 0.9926 | 0.9926 | +0.0039 | ✅ 超越基线 |
            | **FP专家** | 0.9535 | 0.9993 | +0.0106 | ✅ 超越基线 |
            | **FN专家** | 0.9025 | 0.9025 | -0.0862 | ❌ 低于基线 |

            *注：FP专家的OOF AUC（在原始不平衡数据上评估）远高于其5折平均AUC（在平衡数据上评估），这反映了在极度不平衡数据上AUC指标的特性，但其在平衡数据上的0.9535的AUC值已证明其强大的分类能力。*

            **结论**：虽然最初的Ensemble_v2架构因数据泄露不可用，但为解决FP/FN问题而开发的**数据平衡技术**和**专家模型训练框架**是本项目的重要资产。TP、TN、FP三个专家模型均已成功训练并超越基线，其产出的OOF预测（Out-of-Fold Predictions）可以作为未来更严谨的集成学习模型的有效元特征。
...
### 2.4 特征工程与质量评估 (主要代码位于 `src/nationwide/project_all_for_deepling_learning/turn*.py` 和 `src/yangtze/YangTsu/turn*.py`)
...
#### 2.4.1 特征质量与相关性分析 (长江流域)

在完成了长江流域415个展平特征的构建后，我们进行了一次全面的特征质量与相关性分析，以评估特征库的内部质量和冗余度。这一步骤对于后续高效、稳健的模型训练至关重要。

**核心发现:**

*   **数据质量优秀**: 所有415个特征均无缺失值或无穷大值，数据质量非常高。
*   **存在显著冗余**: 分析揭示了特征库内部存在高度的共线性。这在包含大量衍生特征（如基于多产品均值、分位数、滞后项的特征）的复杂特征工程中是正常现象。
    *   **229对** 特征的绝对相关性大于 **0.95**。
    *   **108对** 特征的绝对相关性大于 **0.99**。
*   **冗余来源**: 冗余主要来源于基于相同基础变量（如多产品统计量）构建的衍生特征。例如，`multi_product_mean` 与多个分位数特征 `multi_product_quantile_*` 高度相关。

**关键成果:**

*   生成了完整的 **415x415 相关性矩阵** (`correlation_matrix_full.csv`)。
*   输出了 **高度冗余特征对列表** (`high_correlation_pairs_gt_0.95.csv`) 和 **特征冗余度排名** (`feature_redundancy_ranking.csv`)。
*   所有详细的分析报告、数据和可视化图表均保存在 `results/yangtze/feature_analysis_corrected/` 目录下。

**结论与指导意义:**

这次全面的特征分析是项目的一个重要里程碑。它证明了我们不仅有能力构建大规模特征库，更有能力对其进行深入的质量评估。分析结果明确指出，在进入模型训练的下一阶段之前，必须进行**特征选择**，以消除冗余、降低模型复杂度并提升泛化能力。这些详尽的分析产出为后续的特征选择工作提供了清晰、可靠的数据支持。
...
*   **`src/`**: 核心源代码目录。
    ...
    *   `yangtze/`: 长江流域的降雨预测研究代码，是当前项目的核心工作区域。
        ...
        *   `Ensemble_v2/`: **新增子目录**，用于实现两阶段四元特征融合集成学习策略。
            *   `01_prepare_l1_targets.py`: 根据Level 0模型预测和真实标签，生成TP, FP, FN, TN四种类型的Level 1训练目标。
            *   `02_train_l1_experts.py`: 训练四个Level 1专家模型（分别针对TP, FP, FN, TN），并生成其折外预测作为Level 2融合模型的元特征。
            *   (后续将包含Level 2融合模型脚本及相关产出)
