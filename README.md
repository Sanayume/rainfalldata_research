...
3.  **机器学习与深度学习模型的精细化应用与创新集成：**
    ...
    *   **先进集成学习策略：** 系统性研究并应用如Stacking等高级集成学习框架，将不同子任务模型（分类器、回归器、误差修正模型等）的输出作为元特征，训练更高层次的元学习器，以实现对最终降雨预测结果的优化整合与性能提升。
        *   **第二次尝试：两阶段四元特征融合（Ensemble_v2）**：在 `src/yangtze/Ensemble_v2/` 目录下，我们启动了新的集成学习策略。该策略通过训练四个专家模型（分别针对TP, FP, FN, TN），生成更精细的元特征，以期更有效地修正基础模型的误差，从而提升最终预测的准确性和鲁棒性。
            *   **TP专家（True Positive Expert）**：✅ **已完成训练**
                *   描述：学习识别基础模型何时产生真正例
                *   最佳AUC: **0.994756**（试验编号: 293/300）
                *   训练数据: 4,230,856样本，正样本比例: 61.74%
                *   最优参数: n_estimators=2766, learning_rate=0.0841, max_depth=15, subsample=0.9923, colsample_bytree=0.9468
            *   **TN专家（True Negative Expert）**：✅ **已完成训练**
                *   描述：学习识别基础模型何时产生真负例  
                *   最佳AUC: **0.992657**（试验编号: 91/152）
                *   训练数据: 4,230,856样本，正样本比例: 36.03%
                *   最优参数: n_estimators=2554, learning_rate=0.0833, max_depth=16, subsample=0.9836, colsample_bytree=0.9588
            *   **FP专家（False Positive Expert）**：🔄 **数据平衡训练中**
                *   描述：学习识别基础模型何时产生假正例
                *   问题：原始正样本比例仅2.23%，严重不平衡
                *   解决方案：采用下采样策略，目标30%正样本比例，集成3个平衡子集
                *   状态：已清空原始训练历史，使用新的平衡数据策略重新训练
            *   **FN专家（False Negative Expert）**：🔄 **数据平衡训练中**
                *   描述：学习识别基础模型何时产生假负例
                *   问题：原始正样本比例仅3.72%，严重不平衡
                *   解决方案：采用下采样策略，目标30%正样本比例，集成3个平衡子集
                *   状态：已清空原始训练历史，使用新的平衡数据策略重新训练
            *   提供了专门的Optuna结果分析脚本，可查看各专家模型的超参数优化进展和最佳参数
