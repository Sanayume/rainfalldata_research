...
本研究在多源卫星降水数据预测领域，构建了一套 **宏大而精密的特征工程体系**，其设计理念源于对降雨物理机制的深刻理解与数据驱动模式识别的深度融合。这套体系的核心在于其 **统一的、演进式的特征工程范式**，该范式能够无缝应用于 **全国尺度格点数据** 和 **长江流域局部数据（包括其空间格点子集与离散点位）**。这意味着，无论是针对广袤的全国区域，还是聚焦于特定的长江流域，我们所遵循的特征构建逻辑和迭代策略是一脉相承的。各 `turnX.py` 系列脚本（`turn1.py` 至 `turn6.py`）代表了这一统一范式在不同数据环境下的具体实现与持续优化，其主要区别在于底层数据几何形态（连续格点 vs. 离散点）对某些特征实际计算方式的精细化影响。

本特征库的构建，旨在从多维度捕获降雨过程的复杂性，主要涵盖：

*   **基础降雨观测 (`Raw Values`)**: 作为一切分析的基石，直接纳入各个卫星降水产品在目标时空点上的原始降雨量。这些特征是模型感知降雨事件发生、强度和分布最直接的信号，为后续高级特征的构建提供了原始驱动。
*   **多产品一致性与不确定性 (`Multi-Product Synergy`)**: 鉴于单一卫星产品固有的误差和局限性，我们深入探究多产品间的协同信息。通过计算所有可用产品在当前时空点上的 **平均值、标准差、中位数、最大值、最小值和极差**，我们不仅提供了降雨量的综合估计，也量化了产品间的分散程度，反映了对降雨事件观测的不确定性。更重要的是，我们引入了 **一致降雨产品计数**（即报告降雨量超过 0.1mm 阈值的产品数量），用以衡量不同产品对降雨事件的共识程度，从而在一定程度上增强模型对降雨存在确定性的判断能力。
*   **时序动态演变 (`Temporal Dynamics`)**: 降雨是一个具有显著持续性、周期性和演变性的地球物理过程。我们精心设计了以下时序特征以捕捉其复杂动态，确保模型能从时间维度全面理解降雨行为：
    *   **周期性编码**: 降雨在年内和日内均表现出明确的周期性规律。我们通过将日期信息转换为 **年内周期性的正余弦分量** (`sin_day`, `cos_day`)，使模型能够感知并学习降雨的季节性、昼夜性等宏观循环模式。同时，**季节独热编码** (`season_onehot`) 提供了一种更离散化的季节划分，辅助模型捕捉不同气候季节下迥异的降雨发生和发展模式。
    *   **历史记忆性**: 降雨往往具有“记忆”效应，即当前降雨状况与过去紧密相关。我们引入了 **多步时间滞后特征** (如 t-1, t-2, t-3 天的原始降雨量、产品平均值、标准差、雨量计数)，使模型能够利用过去的降雨信息预测未来。这些特征直接反映了降雨过程的惯性效应和短期自相关性。
    *   **瞬时变化率**: 降雨强度或产品间一致性的快速变化往往预示着事件的发生、增强或消退。我们通过计算 **时间差分特征** (如当前时刻与 t-1 时刻的原始值、产品平均值、标准差的差分)，敏锐地捕捉这些瞬时变化趋势，这对于识别降雨事件的起始、强度波动或结束阶段至关重要。
    *   **累积与波动**: 降雨在不同时间尺度下的累积量和波动性对理解水文响应至关重要。我们设计了 **多尺度滑动窗口统计** (如 3天、7天、15天窗口内的平均值、标准差、最大值、最小值、范围)，这些统计量基于每日产品均值或特定产品序列计算，旨在捕捉不同时间尺度下的降雨累积效应和波动特性，为模型提供更宏观的时间上下文。此外，某些版本还针对特定高性能产品（如 GSMAP 和 PERSIANN）计算了其独立的滑动窗口统计，以捕捉这些产品所反映的独特累积模式。
*   **空间上下文关联 (`Spatial Context`)**: 降雨事件的发生和发展并非孤立点，而是具有明确的局部空间相关性。我们通过邻域统计量尝试捕获这种关联，为模型提供空间上下文信息：
    *   **邻域统计量**: 计算了 **3x3 或 5x5 邻域** 内的降雨产品（或其平均值）的统计量（如均值、标准差、最大值），以反映局部区域的降雨强度、空间异质性和潜在的极端性。
    *   **中心点与邻域差异**: 计算目标格点（或点位）的降雨量与周围邻域统计量（如均值）的差异，这有助于模型理解目标点在局部空间中的相对状态，例如是否是降雨中心或边缘，或者局部降雨梯度是否显著。
    *   **空间梯度**: 针对主要产品（如 GSMAP, PERSIANN）及其平均值，计算了其 **空间梯度幅度**，量化降雨强度在空间上的变化陡峭程度，这对于识别锋面、对流核等强降雨中心区域具有重要的指示意义。
    *   **跨区域适应性**: 值得强调的是，尽管空间特征的设计理念在全国和长江流域是统一的，但其 **实际计算方式会因数据源的底层几何形态而精细化调整**。在处理**全国格点数据**（代码路径 `src/nationwide/...`）以及**长江流域空间格点数据**（通过 `loaddata.py` 的 `get_basin_spatial_data` 获取）时，上述所有空间特征均能通过成熟的图像处理技术（如卷积操作）得到**真实且具有物理意义的计算结果**。然而，在处理**长江流域离散点位数据**（通过 `loaddata.py` 的 `get_basin_point_data` 获取）时，由于点位间缺乏连续网格关系，**这些传统空间特征（如 3x3 邻域、5x5 邻域和空间梯度）将无法直接计算，因此在代码中被务实地填充为 NaN 占位符，并在最终数据展平后转换为 0 值**。这种处理方式既保持了特征结构的统一性，也清晰地反映了点位数据在空间维度上的固有局限性。
*   **弱信号增强与不确定性量化 (`Weak Signal & Uncertainty`)**: 对于易被忽视的低强度降雨（如毛毛雨、小雨），它们虽然强度低，但对水文响应和某些应用场景仍有重要意义。我们设计了多项特征以提升模型的敏感度：
    *   **阈值接近度**: 计算产品平均值与 0.1mm 降雨阈值的绝对距离，使模型能够关注那些“临界”降雨事件，避免在预测时将其简单地归零。
    *   **变异系数 (CV)**: 反映产品标准差与平均值的相对关系，在高不确定性和低平均值时尤为突出，有助于识别低强度降雨下的产品间分歧和固有的不确定性。
    *   **条件不确定性**: 特指当产品平均降雨量低于某一特定阈值（如 1.0mm）时，提取其标准差，从而突出低强度降雨事件中产品间可能存在的较大分歧，为模型提供额外的不确定性信号。
    *   **特定强度分箱**: 将连续的降雨量或产品计数离散化为多个强度区间（如 0mm, 0-0.5mm, >1.0mm 或 0, 1, 2, >=3 个产品报告雨），转化为二值或计数特征。这种离散化有助于模型更清晰地识别和学习不同强度模式下的特定预测行为。
*   **高阶交互特征 (`Interaction Features`)**: 为捕捉多因素间的非线性耦合关系，我们探索性地构建了交互特征。例如，将 `产品标准差` 与 `季节性因子`（如 `sin_day` 的绝对值）相乘，旨在揭示降雨不确定性在不同季节可能表现出的差异性模式；或将 `低强度不确定性` 与 `变异系数` 结合，以更全面、更精细地刻画复杂低雨事件的模糊性。

这套特征集经历了 **至少 6 个主要版本 (`v1` 到 `v6`) 的持续迭代与优化**，其演变过程是基于前一轮模型评估结果和误差分析的数据驱动式决策。每一次版本的调整都旨在提升模型的预测性能，同时也加深了我们对降雨事件物理过程的理解。

---

### **全国和长江流域特征工程的迭代演进细节 (`turn*.py` 系列脚本):**

以下将详细阐述 `turn1.py` 至 `turn6.py` 系列脚本在特征工程上的迭代演进。这套迭代遵循统一的设计理念，并在全国格点数据和长江流域数据（格点子集或离散点位）上展现出各自的实现侧重和适应性。

*   **V1 (`turn1.py`): 统一特征工程范式的奠基者 - 基础框架构建与格点应用**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn1.py`)**: 此版本是全国格点特征工程系列的首个里程碑。它全面加载全国格点数据，并构建了核心特征集：包括基础原始值 (`raw_values`)，以及反映多产品共识度的 `rain_product_count`。在时序动态方面，它引入了季节独热编码 (`season_onehot`) 以捕捉宏观周期性，同时设计了多尺度滞后原始值（`lag_1_values`, `lag_2_values`, `lag_3_values`）和当前与前一天的原始值差分（`diff_1_values`），以捕捉降雨的记忆性与瞬时变化。累积效应方面，实现了基于产品均值的 3, 7, 15 天滑动统计量（均值、标准差、最值、范围）。**尤其值得注意的是，此版本在空间关联方面，通过对格点数据的直接操作，真实计算了 5x5 邻域的均值、标准差和最大值**，这是格点数据独有的优势。V1 作为一个基础版本，特征集相对精简，不包含弱信号增强和高阶交互特征，但其严谨的特征构建模式和对大规模格点数据（利用 `memmap`）的处理能力，为后续的复杂性奠定了坚实的基础。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn1.py`)**: 该脚本在代码逻辑上与全国版本保持了高度一致性。它通过 `mydata().yangtsu()` 接口获取**长江流域的格点数据**（即 `loaddata.py` 中 `X_spatial` 和 `Y_spatial` 的内容，该数据已包含流域外的 NaN 值）。因此，该脚本在长江流域的格点数据上，**忠实地实现了并计算了所有 V1 定义的时序和空间特征，包括真实的 5x5 空间邻域统计**。这体现了早期将全国通用特征工程逻辑直接应用于局部流域格点数据的成功尝试，确保了数据处理的一致性。

*   **V2 (`turn2.py`): 特征维度的初步拓展与精细化 - 增强版格点特征**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn2.py`)**: 此版本在 V1 基础上，对特征集进行了显著拓展与精细化，旨在从更多维度捕获降雨信息。**多产品协同特征得到增强**，除 `rain_product_count` 外，还引入了产品间的变异系数（`coef_of_variation`）和产品极差（`product_range`），以更深入地量化产品间的不确定性。**空间关联特征得到全面丰富**，除了 5x5 邻域的均值、标准差、最大值及中心点与邻域差异（`spatial_center_diff_5x5`），还首次引入了各产品和平均产品的空间梯度（`gradient_magnitude_GSMAP`, `gradient_magnitude_PERSIANN`, `gradient_magnitude_mean`），这些均在全国格点数据上**真实计算**，极大地增强了模型对降雨空间结构变化的感知能力。**弱信号增强特征首次得到引入**，包括距 0.1mm 阈值距离和低量雨产品比例，旨在提升模型对微弱降雨事件的识别能力。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn2.py`)**: 该脚本明确通过 `ALL_DATA.get_basin_spatial_data(basin_mask_value=2)` 获取**长江流域的格点数据**。在长江流域的格点数据上，**完整实现了 V2 定义的所有增强特征，包括真实的空间特征计算**。这进一步体现了统一范式在局部格点数据上的全面应用，为模型提供了更丰富的数据驱动信号。时序动态方面，周期性依然仅含 `season_onehot`，滑动窗口统计包含通用滑动统计，但**移除了 V3 中引入的特定产品滑动窗口统计**（这反映了代码在迭代过程中，特征的引入和移除可能存在跨版本的回溯调整）。

*   **V3 (`turn3.py`): 核心特征集的聚焦与特定产品洞察 - 效率与洞察兼顾**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn3.py`)**: 此版本旨在探索一套更精简、更高效的核心特征集在全国格点数据上的表现。它在 V2 的基础上进行了有策略的裁剪，以优化计算资源并聚焦于核心信息。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn3.py`)**: 明确通过 `ALL_DATA.get_basin_spatial_data(basin_mask_value=2)` 获取**长江流域的格点数据**。其核心思路是**显著简化特征集，以验证一套更精简的核心特征在长江流域格点数据上的有效性**。多产品协同仅保留 `rain_product_count`。时序动态方面，周期性仅含 `season_onehot`（移除了正余弦编码），滞后特征包含原始值、滞后标准差和滞后降雨产品数，时间差分仅为原始值差分。累积效应方面，在通用滑动统计的基础上，**特别引入了针对特定产品（GSMAP 均值，PERSIANN 标准差）的 7 天滑动窗口统计**，这是对高表现力产品独特累积模式的深入洞察。空间关联方面，保留了 5x5 邻域统计量和空间梯度（均在格点数据上真实计算），但移除了 3x3 邻域，体现了特征精简的策略。弱信号增强仅保留基于降雨产品数量的强度分箱。此版本不包含高阶交互特征，强调效率与特定洞察的结合。

*   **V4 (`turn4.py`): 特征库的极致拓展与深度融合 - 综合信息挖掘的巅峰**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn4.py`)**: 代表了全国格点特征工程的**最丰富、最全面、信息密度最高的版本**。它旨在穷尽各种潜在特征组合，以最大化模型的信息获取能力。它集成了之前版本中的大部分有效特征，并进行了全面而深入的深化。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn4.py`)**: 明确通过 `ALL_DATA.get_basin_spatial_data(basin_mask_value=2)` 获取**长江流域的格点数据**。该脚本在长江流域的格点数据上，**完整实现了 V4 定义的所有极致丰富的特征**。**多产品协同特征得到全面涵盖**，包括均值、标准差、中位数、极差等。**时序动态特征得到全面深化**，包括正余弦编码、季节独热编码、更深层次的滞后统计（滞后均值、标准差、雨量计数及它们之间的差分），以及更细致的时间差分。滑动窗口统计涵盖通用和特定产品滑动统计。**空间关联特征达到最全面配置**，包括 3x3 和 5x5 邻域的均值、标准差、最大值及中心点差异，以及所有空间梯度特征，这些都基于格点数据**真实计算**。**弱信号增强和高阶交互特征也得到全面强化**，体现了对复杂降雨模式的多维度、深层次捕捉，成为特征工程领域的一次全面尝试。

*   **V5 (`turn5.py`): 性能与效率的平衡探索 - 实用性优化**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn5.py`)**: 在 V4 达到极致丰富性之后，此版本开始进行特征集的均衡调整，以寻求性能与计算效率之间的最佳平衡，是向更实用化方向迈进的一步。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn5.py`)**: 明确通过 `ALL_DATA.get_basin_spatial_data(basin_mask_value=2)` 获取**长江流域的格点数据**。其核心思路是在 V3 的简化特征集基础上，**重新引入了年内周期正余弦编码**，同时在其他方面保持了相对精简的特征集，旨在找到一个计算效率与模型性能之间的良好平衡点。多产品协同特征与 V3 保持一致（仅 `rain_product_count`）。时序动态特征恢复了 `sin_day`, `cos_day`，并保留 `season_onehot`、滞后原始值、滞后统计量和差分。滑动窗口统计包含通用和特定产品滑动统计。空间关联特征与 V3 保持一致（5x5邻域及梯度，真实计算）。弱信号增强仅保留基于降雨产品数量的强度分箱。此版本不包含高阶交互特征，体现了在保证周期性信息完整性的前提下，对特征集的精简和实用化考量。

*   **V6 (`turn6.py`): 针对离散点位数据的最终优化与实战化 - 适应性精粹**
    *   **全国格点数据场景 (代码路径: `src/nationwide/project_all_for_deepling_learning/turn6.py`)**: 此版本代表了全国格点特征工程的最终整合版本，它在 V4 或 V5 的基础上，进行了最终的特征精选和优化，旨在提供一个兼顾全面性与鲁棒性的特征集，为全国范围内的模型训练提供最终形态的输入。
    *   **长江流域数据场景 (代码路径: `src/yangtze/YangTsu/turn6.py`)**: 这是长江流域特征工程系列中**一个独特且关键的版本**，它明确通过 `ALL_DATA.get_basin_point_data(basin_mask_value=2)` 获取**长江流域的离散点位数据** (`(产品, 时间, 点位索引)`)。
    *   **核心思路**: 该版本旨在构建一套**针对离散点位数据高度优化且具备实战能力的特征体系**。它力求在保持与全国格点特征设计理念一致的同时，**务实处理**了传统空间特征在点位数据上的局限性，并实现了特征的精粹化。**多产品协同和时序动态特征全面恢复**，涵盖了 V4 中所有丰富的维度（包括正余弦编码、季节独热编码、滞后原始值及其均值/标准差、时间差分、以及基于产品均值的通用滑动窗口统计）。
    *   **空间关联方面**，考虑到离散点位数据缺乏连续网格，**传统意义上的 3x3 邻域统计量（均值、标准差、最大值及其与中心点差异）被保留为 NaN 占位符**，并通过代码中的 `np.full(..., np.nan)` 实现，这清晰地指出了其在点位数据上无法直接计算的特性和工程上的应对策略。**此版本明确移除了 5x5 邻域和所有空间梯度特征**，进一步精简了不适用于点位数据的维度，体现了高度的工程务实性。
    *   **弱信号增强和高阶交互特征也全面恢复**，包含了 V4 中所有关键的细节。
    *   **重要性**: V6 代表了长江流域离散点位特征工程的最新和最完善版本。它在保持统一设计理念的同时，针对点位数据的特性进行了精确的裁剪和优化，为基于离散点数据的模型训练提供了鲁棒且高效的特征集，是跨数据类型特征工程适应性的一个典范。
