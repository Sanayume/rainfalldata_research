...
*   **`data/`**: 存放项目所需的所有数据。
    ...
    *   `intermediate/`: 存储经过初步处理（如格式转换、初步清洗）的中间数据。主要用于训练或者特征工程处理的适合从这里加载，这里的数据就经过空间对齐操作的中国地区0.25分辨率144*256矩阵的数据，只有在中国mask区域上的点有值不在中国区域mask上的点为nan
    *   `processed/`: 存储经过完整预处理流程（如 NaN 值填充、时空对齐、掩码应用）后，可直接用于模型输入的最终数据，通常为 `.mat` 文件。其中也包含地理掩码文件（例如在 `processed/.../masks/`）。
*   **`docs/`**: 存放项目相关的各类文档。
...
*   **`scripts/`**: 存放一些辅助性的脚本。
    *   `utils/`: 通用工具脚本。
        *   `safe_handlers.py`: 可能包含处理文件或数据时的安全操作函数。
        *   `encoding_fix.py`: 用于修正或处理不同文件编码问题的脚本。
        *   `torch_safe_load.py`: 安全加载 PyTorch 模型或数据的工具。
        *   `safe_unicode.py`: 处理 Unicode 编码安全的工具。
        *   `memory_utils.py`: 内存管理相关的辅助函数 (当前为空)。
        *   `font_helper.py`: 字体处理相关的辅助脚本，可能用于绘图。
        *   `visualization_helper.py`: 可视化相关的辅助函数。
        *   `gpu_utils.py`: GPU 使用相关的辅助函数，如设备选择、内存监控等。
        *   `scale_utils.py`: 数据缩放或尺度转换相关的工具。
    *   `mask_creation`: 创建地理掩码的脚本。
    *   `ReadGsmapG01d25y0820.m`: 读取特定格式 GSMAP 数据的 MATLAB 脚本。
*   **`src/`**: 核心源代码目录。
    *   `common/`: 通用模块。
        *   `feature_engineering/`: (预期存放) 通用的特征工程函数和类 (当前为空)。
    *   `ensemble_learning/`: 集成学习相关代码。
        *   `ensemble_learning/`: 具体的集成学习流程脚本。
            *   `1_generate_base_predictions.py`: 生成基础模型的预测结果。
            *   `2_train_fn_expert.py`: 训练专门识别漏报 (FN) 的专家模型。
            *   `3_train_fp_expert.py`: 训练专门识别误报 (FP) 的专家模型。
    ...
    *   `nationwide/`: 全国范围的降雨预测研究代码。
        *   `project_all_for_deepling_learning/`: 核心处理流程和实验脚本。
            ...
            *   `get_fronts.py`: 获取帕累托前沿相关的脚本 (可能用于多目标优化)。
            *   (大量 `.png` 文件): 模型评估、特征分析等的可视化结果。
        *   `analysis/`: 针对全国范围研究的分析脚本。
            *   `analyze_feature_sets_v1_v2.py`: 对比分析 v1 和 v2 版本特征集的脚本。
            *   `analyze_correlations_v1.py`, `analyze_correlations_v2.py`: 分析 v1 和 v2 特征集内部特征相关性的脚本。
            *   `analyze_pca.py`: 进行主成分分析 (PCA) 的脚本。
    *   `readdata/`: 各类降雨原始数据的读取和初步解析脚本。
        *   `CHM.py`: 读取和处理 CHM 地面观测融合数据的脚本。
        *   `sm2rain.py`: 读取和处理 SM2RAIN 卫星降雨产品的脚本。
        *   `PERSIANN.py`: 读取和处理 PERSIANN 卫星降雨产品的脚本。
        *   `gsmap.py`: 读取和处理 GSMAP 卫星降雨产品的脚本。
        *   `CMORPH.py`: 读取和处理 CMORPH 卫星降雨产品的脚本。
        *   `IMERG.py`: 读取和处理 IMERG 卫星降雨产品的脚本。
        *   `chirps.py`: 读取和处理 CHIRPS 卫星降雨产品的脚本。
        *   `scale_utils.py`: 数据尺度转换工具函数。
    *   `yangtze/`: 长江流域的降雨预测研究代码。
        *   `YangTsu/` (实际应为 `Yangtze`): 长江流域核心处理流程，脚本结构和功能与 `nationwide/project_all_for_deepling_learning/` 类似。
