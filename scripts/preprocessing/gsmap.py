...
def save_to_excel(data, year, output_path='F:/rainfalldata/GSMAPdata'):
...
for start_idx, end_idx, year in year_info:
    ...
    # 保存数据
    savemat(f'F:/rainfalldata/GSMAPdata/GSMAP_{year}.mat', {'data': china_data})
    print(f"✓ {year}年数据已保存")
...
if input("\n是否需要合并所有年份数据？(y/n): ").lower() == 'y':
    ...
    for _, _, year in year_info:
        data = loadmat(f'F:/rainfalldata/GSMAPdata/GSMAP_{year}.mat')['data']
        all_data.append(data)
    
    all_data = np.concatenate(all_data, axis=2)
    savemat('F:/rainfalldata/GSMAPdata/GSMAP_2016_2020.mat', {'data': all_data})
    print("✓ 合并完成！")
